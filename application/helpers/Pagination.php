...
class Pagination
{
    ...
    private function create_urls($url)
    {
        ...
        if ($this->result_num > $this->per_page) {
            $return .= '<center><div><ul class="pagination">';
            if ($this->pages >= 1) {
                $next = ($this->page - 1);
                $return .= ($next > 0 && $next < $this->pages) ? '<li><a href="' . str_ireplace('#i#', $next, $url) . '">&laquo;</a></li>' : '';
                //for($x = $this->page + 1; $x <= min($this->page + 11, $this->pages); $x++){
                for ($x = max(1, $this->page - 5); $x <= min($this->page + 5, $this->pages); $x++) {
